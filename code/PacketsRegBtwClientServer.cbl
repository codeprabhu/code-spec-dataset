      ...
      *> =========================================================
       ...
       PROCEDURE DIVISION USING LK-STATE LK-DIRECTION LK-PACKET-NAME LK-PACKET-ID.
           EVALUATE LK-STATE
               ...
               WHEN OTHER
                   COPY ASSERT-FAILED
                       REPLACING MSG BY =="Generated-Packets: Invalid client state: " LK-STATE==.
           END-EVALUATE
           ...
           EVALUATE LK-DIRECTION
               ...
               WHEN OTHER
                   COPY ASSERT-FAILED
                       REPLACING MSG BY =="Generated-Packets: Invalid packet direction: " LK-DIRECTION==.
           END-EVALUATE
           ...
           MOVE LK-PACKET-NAME
                TO PACKET-REF-NAME(LK-STATE + 1, LK-DIRECTION + 1, LK-PACKET-ID + 1)
      ...
      *> =========================================================
      *> --- Packets-GetId ---
      *> Look up the numeric ID of a packet by its string reference.
      *> =========================================================
       IDENTIFICATION DIVISION.
       ...
       PROCEDURE DIVISION USING PACKET-REFERENCE PACKET-ID.
           PERFORM VARYING PACKET-INDEX FROM 1 BY 1
               UNTIL PACKET-INDEX > PACKET-ID-COUNT
      ...
      *> =========================================================
      *> --- Packets-GetReference ---
      *> Look up the name of a packet by its client state, direction, and ID.
      *> =========================================================
       IDENTIFICATION DIVISION.
       ...
       WORKING-STORAGE SECTION.
           ...
           COPY DD-PACKET-DIRECTIONS.
       LINKAGE SECTION.
           01  CLIENT-STATE              BINARY-CHAR.
           01  PACKET-DIRECTION          BINARY-LONG UNSIGNED.
           01  PACKET-ID                 BINARY-LONG.
       ...
       PROCEDURE DIVISION USING CLIENT-STATE PACKET-DIRECTION PACKET-ID PACKET-NAME.
           EVALUATE TRUE
               ...
               WHEN NOT (PACKET-DIRECTION = PACKET-DIRECTION-SERVERBOUND
                        OR PACKET-DIRECTION-CLIENTBOUND)
               ...
               WHEN PACKET-ID < 0 OR >= PACKET-REF-ENTRY-LENGTH
                   ...
                   MOVE SPACES TO PACKET-NAME
               WHEN OTHER
                   MOVE PACKET-REF-NAME(CLIENT-STATE + 1, PACKET-DIRECTION + 1, PACKET-ID + 1)
                        TO PACKET-NAME
           END-EVALUATE
           GOBACK.
      ...
      *> Register a handler for a specific client state + packet ID.
      *> =========================================================
       IDENTIFICATION DIVISION.
       ...
       PROCEDURE DIVISION USING LK-CLIENT-STATE LK-PACKET-NAME LK-HANDLER-NAME.
           ...
           EVALUATE LK-CLIENT-STATE
               WHEN CLIENT-STATE-HANDSHAKE
                   STRING "handshake/serverbound/"
                          FUNCTION TRIM(LK-PACKET-NAME)
                          INTO PACKET-REFERENCE
               WHEN CLIENT-STATE-STATUS
                   STRING "status/serverbound/"
                          FUNCTION TRIM(LK-PACKET-NAME)
                          INTO PACKET-REFERENCE
               WHEN CLIENT-STATE-LOGIN
                   STRING "login/serverbound/"
                          FUNCTION TRIM(LK-PACKET-NAME)
                          INTO PACKET-REFERENCE
               WHEN CLIENT-STATE-CONFIGURATION
                   STRING "configuration/serverbound/"
                          FUNCTION TRIM(LK-PACKET-NAME)
                          INTO PACKET-REFERENCE
               WHEN CLIENT-STATE-PLAY
                   STRING "play/serverbound/"
                          FUNCTION TRIM(LK-PACKET-NAME)
                          INTO PACKET-REFERENCE
               WHEN OTHER
                   COPY ASSERT-FAILED
                       REPLACING MSG BY =="Error: RegisterPacketHandler: Invalid client state " LK-CLIENT-STATE==.
           END-EVALUATE
           ...
           MOVE HANDLER-PTR TO PACKET-HANDLER(LK-CLIENT-STATE + 1, LK-PACKET-ID + 1)
           GOBACK.
       END PROGRAM RegisterPacketHandler.

      *> =========================================================
      *> --- HandlePacket ---
      *> Handle an incoming packet from a connected client.
      *> =========================================================
       IDENTIFICATION DIVISION.
       ...
       WORKING-STORAGE SECTION.
           ...
           COPY DD-PACKET-DIRECTIONS.
           01  PACKET-DIRECTION          BINARY-LONG UNSIGNED
                                         VALUE PACKET-DIRECTION-SERVERBOUND.
           01  HANDLER-PTR               PROGRAM-POINTER.
       ...
       PROCEDURE DIVISION USING LK-CLIENT LK-CLIENT-STATE LK-PACKET-ID
                               LK-PAYLOAD LK-PAYLOAD-LENGTH LK-OFFSET.
           ...
           MOVE PACKET-HANDLER(LK-CLIENT-STATE + 1, LK-PACKET-ID + 1)
                TO HANDLER-PTR
           ...
           IF HANDLER-PTR NOT = NULL
               ...
               IF CONSUMED NOT = LK-PAYLOAD-LENGTH
                   CALL "Packets-GetReference"
                        USING LK-CLIENT-STATE PACKET-DIRECTION LK-PACKET-ID PACKET-REFERENCE
                   DISPLAY "[state=" LK-CLIENT-STATE "] Packet "
                       FUNCTION TRIM(PACKET-REFERENCE)
                       " handler consumed " CONSUMED " of "
                       LK-PAYLOAD-LENGTH " bytes"
               END-IF
           END-IF
           GOBACK.
      ...
      *> =========================================================
      *> --- SendPacket ---
      *> Send a raw packet to the client socket.
      *> =========================================================
       IDENTIFICATION DIVISION.
       ...
       PROCEDURE DIVISION USING LK-CLIENT LK-PACKET-ID LK-PAYLOAD LK-PAYLOAD-LENGTH.
           ...
           CALL "Encode-GetVarIntLength"
                USING LK-PACKET-ID NUM-BYTES
